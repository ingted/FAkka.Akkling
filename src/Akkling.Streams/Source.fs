...
module Source =
    ...
    let inline singleton (elem: 't) = //: Source<'t, unit> = 
        Source.Single(elem).MapMaterializedValue(Func<NotUsed, Akka.NotUsed>(fun _ -> Akka.NotUsed.Instance))
    ...
    /// downstream.
    let inline watchTermination (matFn: 'mat -> Async<Akka.Done> -> 'mat2) (source) : Source<'t, 'mat2> =
